...
! this only applies if sink particles are present
...
#endif
        ! clear the sink->gas accelerations (filled by Particles_sinkAccelGasOnSinksAndSinksOnGas below)
        solnVec(SGAX_VAR,:,:,:) = 0.0
        solnVec(SGAY_VAR,:,:,:) = 0.0
        solnVec(SGAZ_VAR,:,:,:) = 0.0
#endif
...
! This only gets called if there are active particles.
  ...
  density = PDEN_VAR
#ifdef DENS_VAR
  if (Debug) print *, 'Gravity_potentialListOfBlocks: adding DENS_VAR to PDEN_VAR.'
  do lb = 1, blockCount
     call Grid_getBlkPtr(blocklist(lb), solnVec)
  ...
  if (Debug) print *, 'Gravity_potentialListOfBlocks: before Grid_solvePoisson'
  invscale=grav_poisfact*invscale
  call Grid_solvePoisson (GPOT_VAR, density, bcTypes, bcValues, &
       invscale)
  if (Debug) print *, 'Gravity_potentialListOfBlocks: after Grid_solvePoisson'

! Un-junk PDEN if it exists and if requested.

#ifdef PDEN_VAR
  if (grav_unjunkPden) then
     if (Debug) print *, 'Gravity_potentialListOfBlocks: grav_unjunkPden=true, removing DENS_VAR from PDEN_VAR.'
     density = PDEN_VAR
#ifdef DENS_VAR
     do lb = 1, blockCount
        call Grid_getBlkPtr(blocklist(lb), solnVec)
