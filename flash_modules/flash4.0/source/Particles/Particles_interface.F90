...
#include "constants.h"
  ...
  interface
     subroutine Particles_mapFromMesh (mapType, numAttrib, attrib, pos, plev, bndBox, &
          deltaCell, solnVec,  partAttribVec)

       integer, INTENT(in) :: mapType, numAttrib, plev
  ...
  interface
    subroutine Particles_sinkSupernova(dt)
      real, intent(IN) :: dt
    end subroutine Particles_sinkSupernova
  end interface
  ...
  interface
    subroutine Particles_PICComputeDt(blockID,dt_pic,dt_minloc)
      integer, INTENT(in)    :: blockID
      real, INTENT(inout)    :: dt_pic
      integer, INTENT(inout) :: dt_minloc(5)
    end subroutine Particles_PICComputeDt
  end interface

  interface
    subroutine Particles_CRComputeDt(blockID,dt_cr,dt_minloc)
      integer, INTENT(in)    :: blockID
      real, INTENT(inout)    :: dt_cr
      integer, INTENT(inout) :: dt_minloc(5)
    end subroutine Particles_CRComputeDt
  end interface

  interface
    subroutine Particles_massiveMarkRefineDerefine()
    end subroutine Particles_massiveMarkRefineDerefine
  end interface

  interface
    subroutine Particles_splitting(move_particles)
      logical, intent(in) :: move_particles
    end subroutine Particles_splitting
  end interface

  interface
    subroutine Particles_dynamicCreationDestruction(move_particles)
      logical, intent(in) :: move_particles
    end subroutine Particles_dynamicCreationDestruction
  end interface

  interface
    subroutine Particles_setTag(ip, tag)
      integer, intent(IN) :: ip
      real*8, intent(IN) :: tag
    end subroutine Particles_setTag
  end interface

  interface
    real*8 function Particles_getTag(ip)
      integer, intent(IN) :: ip
    end function Particles_getTag
  end interface
