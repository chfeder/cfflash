...
subroutine Particles_computeDt (blockID, dt_part, dt_minloc)
  ...
  use Particles_interface, ONLY: Particles_sinkComputeDt, Particles_PICComputeDt, Particles_CRComputeDt
  implicit none
...
#include "constants.h"
  ...
  do i = 1, pt_numLocal
    ...
    if (lb .eq. blockID) then
      ...
      if (velxabs .gt. pt_small) then
        dtx = delta(1) / velxabs
      endif

#if NDIM > 1
      velyabs = abs(particles(VELY_PART_PROP,i))
      if (velyabs .gt. pt_small) then
        dty = delta(2) / velyabs
      endif
#endif
      ...
      if (velzabs .gt. pt_small) then
        dtz = delta(3) / velzabs
      endif
#endif

      !! decrease by the safety factor
      dtnew = pt_dtFactor * min(dtx, dty, dtz)
      if (dtnew .lt. dt_part) then
          dt_part = dtnew
  ...
  ! sink particle time step
  call Particles_sinkComputeDt(blockID,dt_part,dt_minloc)

  ! particle-in-cell (PIC) particle timestep
  call Particles_PICComputeDt(blockID,dt_part,dt_minloc)

  ! cosmic ray (CR) particle timestep
  call Particles_CRComputeDt(blockID,dt_part,dt_minloc)
