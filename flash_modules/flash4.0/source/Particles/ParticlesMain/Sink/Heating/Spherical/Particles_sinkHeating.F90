!!****if* source/Particles/ParticlesMain/Sink/Heating/Particles_sinkHeating
!!
!! NAME
!!
...
#include "Particles.h"
    ...
    real, save    :: sigma_sb, c, sb_factor, norm_factor
    real, save    :: delta_at_lrefmax, heating_time_char
    logical, save :: first_call = .true.

    integer, save :: sink_heating_model
    real, save    :: sink_heating_radius, sink_heating_inner_radius, sink_heating_luminosity_fraction

    integer :: blockCount, lb, blockID, size_x, size_y, size_z, i, j, k, p
    real    :: heating_radius_comoving, redshift, theat4, dx, dy, dz, radius, lum
    real    :: t_shape
...
#endif
        sb_factor = 1.0/(16.0*PI*sigma_sb)
    ...
    if ((sink_heating_model .le. 0) .or. (localnpf .le. 0)) return
        ...
        do k = blkLimits(LOW,KAXIS), blkLimits(HIGH,KAXIS)
          do j = blkLimits(LOW,JAXIS), blkLimits(HIGH,JAXIS)
            do i = blkLimits(LOW,IAXIS), blkLimits(HIGH,IAXIS)
...
#endif
                         ! to scale the radial dependence of luminosity heating
                         norm_factor = 1.0/sink_heating_inner_radius + 1.0/sink_heating_radius
                         theat4 = theat4 + t_shape * lum / ((radius/sink_heating_inner_radius+1.0)/norm_factor)**2
               ...
               solnData(SLHT_VAR,i,j,k) = (sb_factor*theat4)**0.25
