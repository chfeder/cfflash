!!****if* source/Particles/ParticlesMain/Sink/pt_sinkAccelSinksOnSinks
!!
!! NAME
!!
...
!! NOTES
!!
!!   written by Christoph Federrath, 2008-2021
...
#include "Particles.h"
    ...
    integer, dimension(gather_nprops), save :: gather_propinds = &
      ...
      ! ACCX_PART_PROP, ACCY_PART_PROP, ACCZ_PART_PROP, MASS_PART_PROP do not have to be
      ! communicated, because they are updated in Particles_sinkAdvanceParticles
    ...
    if (first_call) then
      ...
      if ((localnpf .ge. 100) .and. sink_AdvanceSerialComputation .and. (dr_globalMe .eq. MASTER_PE)) print *, &
        & "pt_sinkAccelSinksOnSinks: many sink particles present; consider setting sink_AdvanceSerialComputation = .false."
    ...
    ! if there is only 1 sink particle we still do this to fill up local_max_accel (then only due to gas gravity),
    ! so the (sub-cycling) time step is computed correctly in pt_sinkGetSubCycleTimeStep
    if (localnpf .eq. 0) return

    call Timers_start("AccelSinksOnSinks")
    ...
    if (sink_AdvanceSerialComputation) then
      ...
      ! positions of all particles (call to pt_sinkGatherGlobal),
      np = localnp
      ...
      do pf = 1, localnpf ! this always goes over all sink particles
        ...
        if (pf .ne. p) then ! exclude self-interaction
          ...
          if (radius .lt. softening_radius_comoving) then
            if (softening_type_sinks .eq. 1) then ! spline softening
              q = radius*hinv
              kernelvalue = 0.0
            ...
            if (softening_type_sinks .eq. 2) then ! linear kernel
              ax = dx*slope
              ay = dy*slope
              az = dz*slope
            end if
    ...
    call Timers_stop("AccelSinksOnSinks")
