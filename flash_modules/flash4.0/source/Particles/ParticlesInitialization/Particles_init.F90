...
subroutine Particles_init ( restart)
  ...
  use pt_interface, ONLY : pt_mapStringParamToInt, pt_picInit, pt_crInit, pt_setMask, pt_setDataStructures
...
!-------------------------------------------------------------------------------
  ...
  call RuntimeParameters_get ("pt_splitting", pt_splitting)
  call RuntimeParameters_get ("pt_splitting_min_smooth_len", pt_splitting_min_smooth_len)
  ...
  pt_reduceGcellFills = .false. !Works with passive particles only
#endif
  ...
  ! initialize particle-in-cell (PIC) particles
  call pt_picInit()

  ! initialize cosmic ray (CR) particles
  call pt_crInit()
