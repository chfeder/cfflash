...
subroutine IO_writeParticles(particlesToCheckpoint)
  ...
  use IO_interface, ONLY : IO_updateScalars, IO_checkForUserDump
...
#include "Particles.h"
 ...
 ! for logfile output
  ...
  real*8        :: lsimTime   !local sim time variable
  ...
  if(particlesToCheckpoint) then
     ...
     if ((io_particleFileIntervalTime > 0.e0 .AND. lsimTime >= io_nextParticleFileTime) &
          ...
          (io_nextParticleFileStep == nstep) .OR. IO_checkForUserDump(lsimTime, 'part') .OR. &
